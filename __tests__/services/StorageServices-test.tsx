...
import { mockDataAPI } from "../mockData/mockDataAPI.ts";
import { mockDataAsyncStorage } from "../mockData/mockDataAsyncStorage.ts";
...
describe("StorageService Functions", () => {
  ...
  describe("getUserInfo", () => {
    ...
    it("should throw an error if AsyncStorage.getItem fails", async () => {
      ...
      // Act and Assert
      await expect(StorageService.getUserInfo()).rejects.toThrow(
        "AsyncStorage error",
      );
    });
  ...
  describe("Course List", () => {
    ...
    it("should return the course list from AsyncStorage if it exists", async () => {
      ...
      jest
        .spyOn(api, "getCourses")
        .mockResolvedValueOnce(mockData.allCoursesData);
    ...
    it("should handle errors in refreshCourseList", async () => {
      const errorMessage =
        "Error getting course list from database: " + mockData.errorResponse;
  ...
  describe("getSectionList", () => {
    it("should get the section list from API and refresh it in AsyncStorage", async () => {
      ...
      jest
        .spyOn(StorageService, "refreshSectionList")
        .mockImplementationOnce((list) => list);
    ...
    it("should use locally stored sections if API fails and local data exists", async () => {
      ...
      const mockSectionList = [{ title: "Section 1" }, { title: "Section 2" }];
      jest
        .spyOn(api, "getAllSections")
        .mockRejectedValueOnce(new Error("API error"));
      await AsyncStorage.getItem.mockResolvedValueOnce(
        JSON.stringify(mockSectionList),
      );
    ...
    it("should return an empty array if both API and local storage fail", async () => {
      ...
      jest
        .spyOn(api, "getAllSections")
        .mockRejectedValueOnce(new Error("API error"));
      await AsyncStorage.getItem.mockRejectedValueOnce(
        new Error("AsyncStorage error"),
      );
      const result = await StorageService.getSectionList("course_id");
      // Act and Assert
  ...
  describe("getComponentList", () => {
    ...
    it("should use locally stored components if API fails and local data exists", async () => {
      ...
      jest
        .spyOn(api, "getComponents")
        .mockRejectedValueOnce(new Error("API error"));
      await AsyncStorage.getItem.mockResolvedValueOnce(
        JSON.stringify(mockComponentList),
      );
  ...
  describe("fetchLectureImage", () => {
    it("should fetch lecture image from API and return it", async () => {
      ...
      // Act
      const result = await StorageService.fetchLectureImage(
        mockImageID,
        mockLectureID,
      );
  ...
  describe("Subscribed Course List", () => {
    ...
    it("should return the subscribed course list from AsyncStorage if it exists", async () => {
      ...
      AsyncStorage.getItem.mockResolvedValue(
        JSON.stringify(mockDataAsync.subscribedCourses2),
      );

      // Mock the api.getSubscriptions function
      jest
        .spyOn(api, "getSubscriptions")
        .mockResolvedValue(mockData.subscribedCourses2);
    ...
    it("should call refreshSubCourseList and return its result if courses are not in AsyncStorage", async () => {
      ...
      jest
        .spyOn(StorageService, "refreshSubCourseList")
        .mockResolvedValue(mockDataAsync.subscribedCourses);

      // Mock the api.getSubscriptions function
      jest
        .spyOn(api, "getSubscriptions")
        .mockResolvedValue(mockData.subscribedCourses);
    ...
    it("should throw an error if AsyncStorage getItem fails", async () => {
      ...
      AsyncStorage.getItem.mockRejectedValue(
        new Error("Cannot fetch user id from async storage"),
      );

      await expect(StorageService.getSubCourseList()).rejects.toThrow(
        "Cannot fetch user id from async storage",
      );
    });
  });
  ...
  describe("subscribe", () => {
    ...
    it("should subscribe the user to a course", async () => {
      ...
      expect(api.subscribeToCourse).toHaveBeenCalledWith(
        mockData.userData._id,
        mockData.courseData._id,
      );
    });
    ...
    it("should throw an error when AsyncStorage getItem fails", async () => {
      ...
      await expect(
        StorageService.subscribe(mockData.courseData._id),
      ).rejects.toThrow("Cannot fetch user id from async storage");
    ...
    it("should throw an error when api.subscribeToCourse fails", async () => {
      ...
      api.subscribeToCourse.mockRejectedValue(
        new Error("API error in subscribe:"),
      );

      // Assert that the subscribe function throws the expected error
      await expect(
        StorageService.subscribe(mockData.courseData._id),
      ).rejects.toThrow("API error in subscribe:");
  ...
  describe("unsubscribe", () => {
    ...
    it("should unsubscribe the user to a course", async () => {
      ...
      // Mock the successful behavior of api.subscribeToCourse
      await api.unSubscribeToCourse.mockResolvedValue(
      ...
      // Assert that api.subscribeToCourse was called with the correct arguments
      expect(api.unSubscribeToCourse).toHaveBeenCalledWith(
        mockData.userData._id,
        mockData.courseData._id,
      );
    });
    ...
    it("should throw an error when AsyncStorage getItem fails", async () => {
      ...
      // Assert that the subscribe function throws the expected error
      await expect(
        StorageService.unsubscribe(mockData.courseData._id),
      ).rejects.toThrow("Cannot fetch user id from async storage");
    ...
    it("should throw an error when api.subscribeToCourse fails", async () => {
      ...
      // Mock the behavior of api.subscribeToCourse to simulate a failure
      api.unSubscribeToCourse.mockRejectedValue(
  ...
  describe("deleteLocallyStoredCourse", () => {
    it("should delete a locally stored course and return true", async () => {
      ...
      const mockSectionList = [
        { _id: "section_id_1", lectures: [{ _id: "lecture_id_1" }] },
        { _id: "section_id_2", lectures: [{ _id: "lecture_id_2" }] },
      ];
      ...
      // Act
      const result =
        await StorageService.deleteLocallyStoredCourse(mockCourseID);
      /*
      // Assert
      expect(result).toBe(true);
...
*/
    ...
    it("should handle errors and return false", async () => {
      ...
      await AsyncStorage.getItem.mockRejectedValue(
        new Error("AsyncStorage error"),
      );
  ...
  describe("updateStoredCourses", () => {
    ...
    it("should handle errors during course update", async () => {
      ...
      // Mock API functions
      jest
        .spyOn(StorageService, "storeCourseLocally")
  ...
  describe("Video Handling Functions", () => {
    ...
    describe("deleteLectureVideo", () => {
      ...
      it("should handle deletion errors appropriately", async () => {
        ...
        // Act & Assert
        await expect(
          StorageService.deleteLectureVideo(videoName),
        ).rejects.toThrow("Error in deleteLectureVideo: Error: Delete error");
      });
    });
  });
});
