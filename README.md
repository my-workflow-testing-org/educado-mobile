...
_Since the app is designed for android, using an android emulator is strongly recommended_
...
_Since the app is designed for android, this is not recommended_
...
_Can only be tested on your own phone or emulator when the app is downloaded, not with expo go_
_However, as an alternative, if you are running on local backend, you can dissconnect the backend to simulate no internet connection_
